...
<template>
  <div class="ch-file-upload" :class="{ 'ch-file-upload--disabled': disabled }">
    ...
    >
      ...
      >
        <path
          d="M10.667 21.333A5.333 5.333 0 018 11a6.667 6.667 0 0112.933-2.267A5.333 5.333 0 0124 19.333"
        />
      ...
      <button
        type="button"
        class="ch-file-upload__browse-btn"
        :disabled="disabled"
        :aria-label="`${label ? `${label}: ` : ''}${buttonText}`"
        @click="onBrowseClick"
      >
        {{ buttonText }}
      </button>
    ...
    <ul v-if="files.length > 0" class="ch-file-upload__list" role="list">
      ...
      >
        <svg
          class="ch-file-upload__file-icon"
          width="16"
          height="16"
          viewBox="0 0 16 16"
          fill="none"
